...
export class Renderer {
  ...
  protected renderCluster(cluster: ICluster): string {
    ...
    const id = cluster.id !== undefined
      ? renderAttributeValue(cluster.id)
      : undefined;
    // attributes
    ...
    return joinLines(
      ...
      contents.length > 0 ? indent(contents) : undefined,
      "}",
    );
  }
  protected renderRootCluster(rootCluster: IRootCluster): string {
    ...
    return joinLines(
      comment,
      concatWordsWithSpace(rootCluster.strict ? "strict" : undefined, cluster),
    );
  }

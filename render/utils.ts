...
export function leftPadFactory(
  pad: string,
...
export function concatWordsFactory(
  deciliter: string,
): (...lines: (string | undefined)[]) => string {
  return (...lines: (string | undefined)[]): string =>
    lines.filter((l) => typeof l === "string").join(deciliter);
}
...
export function renderAttributeValue(value: AttributesValue): string {
  ...
  let stringValue: string = typeof value === "string"
    ? value
    : value.toString();
  if (isNotString) {
    isHTMLLike = false;
...
export function renderAttributeBuilder<T extends string>(
  ...
  return ([key, value]): string | undefined =>
    join(key, " = ", renderAttributeValue(value), deciliter);
}
  ...
  return joinLines(
    ...
    indent(
      joinLines(
        ...
        ...attributes.values.map(renderAttributeWithComma),
      ),
    ),
    "]",
  );

 ...
 * @hidden
 */
export abstract class Cluster<T extends string> extends AttributesBase<T>
  ...
  public getSubgraph(id: string): ISubgraph | undefined {
    return Array.from(this.objects.subgraphs.values()).find((subgraph) =>
      subgraph.id === id
    );
  }
  /**
  ...
  public subgraph(...args: unknown[]): ISubgraph {
    const id = args.find((arg: unknown): arg is string =>
      typeof arg === "string"
    );
    const attributes = args.find((
      arg: unknown,
    ): arg is ClusterSubgraphAttributes =>
      typeof arg === "object" && arg !== null
    );
    const callback = args.find((
      arg: unknown,
    ): arg is (subgraph: ISubgraph) => void => typeof arg === "function");
    const subgraph: ISubgraph = id
  ...
  public node(id: string, ...args: unknown[]): INode {
    const attributes = args.find((arg: unknown): arg is NodeAttributes =>
      typeof arg === "object" && arg !== null
    );
    const callback = args.find((arg: unknown): arg is (node: INode) => void =>
      typeof arg === "function"
    );
  ...
  public edge(targets: EdgeTargetLike[], ...args: unknown[]): IEdge {
    ...
    const callback = args.find((arg: unknown): arg is (edge: IEdge) => void =>
      typeof arg === "function"
    );
...
export class Subgraph
  extends Cluster<attribute.Subgraph | attribute.ClusterSubgraph>
